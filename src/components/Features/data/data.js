export const data={
    user1:{
        h1:"1000+ HAPPY CAREERS WITH OUR PERSONALIZED MENTORSHIP + TRAINING",
        p:" Tier-3 College to 30+ LPA",
        details:` Copper mug try-hard pitchfork pour-over freegan heirloom neutra
        air plant cold-pressed tacos poke beard tote bag. Heirloom echo
        park mlkshk tote bag selvage hot chicken authentic tumeric
        truffaut hexagon try-hard chambray.`,
        company:" COMMVALT'",
        img:'https://dummyimage.com/720x600'

    },
    user2:{
        h1:"1000+ HAPPY CAREERS WITH OUR PERSONALIZED MENTORSHIP + TRAINING",
        p:" Tier-3 College to 34+ LPA",
        details:` Copper mug try-hard pitchfork pour-over freegan heirloom neutra
        air plant cold-pressed tacos poke beard tote bag. Heirloom echo
        park mlkshk tote bag selvage hot chicken authentic tumeric
        truffaut hexagon try-hard chambray.`,
        company:" COMMVALT'",
        img:'https://nextjs.org/_next/image?url=https%3A%2F%2Fwww.datocms-assets.com%2F35255%2F1665059775-delba.jpg&w=640&q=75'

    },
    user3:{
        h1:"1000+ HAPPY CAREERS WITH OUR PERSONALIZED MENTORSHIP + TRAINING",
        p:" Tier-3 College to 40+ LPA",
        details:` Copper mug try-hard pitchfork pour-over freegan heirloom neutra
        air plant cold-pressed tacos poke beard tote bag. Heirloom echo
        park mlkshk tote bag selvage hot chicken authentic tumeric
        truffaut hexagon try-hard chambray.`,
        company:" COMMVALT'",
        img:'https://leerob.io/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Favatar.b1d1472f.jpg&w=256&q=75'

    }
}